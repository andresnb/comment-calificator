...
def main
  ...
  (1..total_students).each do |student|
    ...
    evaluation.total = evaluation.assign_totals(matrixes: [totals_matrix, totals_student], keys: keys, sheet: evaluations)
    evaluation.dev_skills = evaluation.assign_totals(matrixes: [dev_skills_matrix, dev_skills_student], keys: keys, sheet: evaluations)
    evaluation.user_stories = evaluation.assign_totals(matrixes: [user_stories_matrix, user_stories_student], keys: keys, sheet: evaluations)
    evaluation.optional = evaluation.assign_totals(matrixes: [optional_matrix, optional_student], keys: keys, sheet: evaluations)
